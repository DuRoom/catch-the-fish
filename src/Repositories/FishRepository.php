...
use Carbon\Carbon;
use DuRoom\CatchTheFish\Fish;
use DuRoom\CatchTheFish\Round;
use DuRoom\CatchTheFish\Validators\FishImageValidator;
use DuRoom\CatchTheFish\Validators\FishValidator;
...
class FishRepository
{
     ...
     * @throws \DuRoom\Foundation\ValidationException
     */
    ...
    public function storeDefaultData(Round $round): void
    {
        ...
        foreach (self::BASE_IMAGES as $index => $originalImagePath) {
            ...
            $fish->name = $this->translator->trans('duroom-catch-the-fish.api.default-fish-name', [
                '{number}' => $index + 1,
            ]);
            Placement::random()->assign($fish);
    ...
    /**
     * Resolve Intervention Image manager from container so DuRoom picks driver between GD or Imagick
     * @return ImageManager
     */
